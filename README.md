...
Copy `.env.example` to `.env` and fill in your credentials, then start the backend (see `backend/README.md`).

## Deploy on Vercel

[![Deploy with Vercel](https://vercel.com/button)](https://vercel.com/new)

Set the required environment variables in the Vercel Dashboard (refer to `.env.example` for the list).
...
- **Database**: PostgreSQL required. Falls back to offline mock data if unavailable.
- **Auth**: Supports Google, LINE, and Facebook OAuth. Falls back to demo mode without credentials.
