...
export const TasksView: React.FC = () => {
  ...
  const handleCreate = () => {
    ...
    addTask({
      ...
      assignee: { name: form.assigneeName || 'Me', avatar: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=150&auto=format&fit=crop&q=80' },
    });
  ...
  const grouped = (groupBy: Task['status'][]) => {
    return groupBy.map((status) => ({
      status,
      label: status === 'todo' ? 'To Do' : status === 'in_progress' ? 'In Progress' : 'Done',
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h3 className="text-lg font-extrabold text-slate-900">Tasks</h3>
          <p className="text-xs text-slate-500">{tasks.filter((t) => t.status !== 'done').length} open · {tasks.filter((t) => t.status === 'done').length} completed</p>
        </div>
        <div className="flex items-center gap-3 flex-wrap">
          <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-xl px-3 py-1.5 text-xs font-medium">
            ...
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search tasks..." className="bg-transparent font-medium text-slate-800 focus:outline-none w-32 placeholder:text-slate-300" />
          </div>
          <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-xl px-3 py-1.5 text-xs font-medium">
          ...
          <button onClick={() => setShowForm(true)} className="bg-yellow-500 hover:bg-yellow-600 text-white font-bold px-4 py-2 rounded-xl text-xs shadow-md rf-yellow-glow flex items-center gap-1.5 transition-all hover:scale-105">
            <Plus className="w-4 h-4 stroke-[3]" /><span>Add Task</span>
          </button>
        </div>
      </div>
        ...
        {grouped(['todo', 'in_progress', 'done']).map((col) => (
          <div key={col.status} className="min-w-[300px] w-[300px] shrink-0 flex flex-col">
            ...
            <div className="flex-1 rounded-2xl border border-slate-200/80 bg-slate-50/50 p-2.5 space-y-2.5">
              {col.tasks.length === 0 ? (
                <div className="h-24 border-2 border-dashed border-slate-200/60 rounded-xl flex items-center justify-center text-slate-400 text-xs font-medium">No tasks</div>
              ) : (
                col.tasks.map((task) => (
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setShowForm(false)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md" onClick={(e) => e.stopPropagation()}>
            <div className="p-5 border-b border-slate-100">
              <h3 className="text-base font-extrabold text-slate-900">New Task</h3>
            </div>
            <div className="p-5 space-y-4">
              <input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="Task title *" className="w-full text-sm font-bold bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 focus:outline-none focus:border-blue-400" autoFocus />
              <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Description (optional)" className="w-full text-xs bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 focus:outline-none focus:border-blue-400 resize-none h-20" />
              ...
              <input value={form.assigneeName} onChange={(e) => setForm({ ...form, assigneeName: e.target.value })} placeholder="Assignee name (optional)" className="w-full text-xs bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 focus:outline-none focus:border-blue-400" />
              <div className="flex gap-3 pt-2">
                <button onClick={() => setShowForm(false)} className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold text-xs px-4 py-2.5 rounded-xl transition-all">Cancel</button>
                <button onClick={handleCreate} disabled={!form.title.trim()} className="flex-1 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-300 text-white font-bold text-xs px-4 py-2.5 rounded-xl shadow-md transition-all">Create Task</button>
              </div>
            </div>
          </div>
        </div>
